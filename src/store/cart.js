...
export const useCartStore = defineStore("cart", {
    ...
    // геттеры (без аргументов, кэшируются)
    getters: { 
     ...
     // Это метод (принимает аргумент, отлично подходит для v-if)
    actions: {
        addToCart(product) {
            const currentItemIndex = this.items.findIndex((item) => item.product.id === product.id); // используется метод findIndex, кт ищет индекс первого элемента массива (this.items), у кт product.id равен id добавляемого товара. Если найден подходящий элемент, existingItemIndex получит его индекс, иначе — -1.
